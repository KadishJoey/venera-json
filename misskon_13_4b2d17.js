...
class ConvertedSource_misskon_13_4b2d17 extends ComicSource {
  name = "MissKon图集";
  key = "misskon_13_4b2d17";
  version = "0.1.0";
  minAppVersion = "1.0.0";
  url = "";
  baseUrl = "https://misskon.com";
  defaultHeaders = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "Referer": "https://misskon.com"};
  legadoRules = {"ruleSearchUrl":"https://misskon.com/page/searchPage/?s=searchKey","ruleSearchList":"article.item-list","ruleSearchName":"h2.post-box-title a@text","ruleSearchAuthor":"","ruleSearchNoteUrl":"h2.post-box-title a@href","ruleSearchCoverUrl":"img.lazy@data-src","ruleSearchKind":"","ruleSearchLastChapter":"","ruleBookName":"h1.name@text","ruleBookAuthor":"","ruleBookKind":"","ruleBookLastChapter":"","ruleIntroduce":"","ruleCoverUrl":"[id=the-post] .entry img@data-src","ruleChapterList":"@js:(function(){try{var m=String(result).match(/rel=\"canonical\" href=\"([^\"]+)\"/);var base=m&&m[1]?m[1]:'';if(!base){return ['MISSKON_ERR:no-canonical head='+String(result).substring(0,60)];}if(/\\/\\d+\\/$/.test(base)){base=base.replace(/\\/\\d+\\/$/,'/');}var max=1,re=/class=\"post-page-numbers\"[^>]*>(\\d+)<\\/[as]>/g,mm;while(mm=re.exec(result)){var n=parseInt(mm[1]);if(n>max){max=n;}}var out=[];for(var i=1;i<=max;i++){out.push({title:'第'+i+'页',url:i===1?base:base+i+'/'});}return out;}catch(e){return ['MISSKON_ERR:'+e.message+' head='+String(result).substring(0,80)];}})()","ruleChapterName":"@js:result.title","ruleChapterUrl":"","ruleContentUrl":"@js:result.url","ruleBookContent":"@js:(function(){try{var body=String(result);var em=body.match(/<div class=\"entry\">([\\s\\S]*?)(?:<div class=\"clear\">|<\\/article>)/);var seg=em&&em[1]?em[1]:body;var out=[];var re=/data-src=\"(https:\\/\\/[^\"]+\\.misskon\\.com\\/(?:uploads|images)\\/[^\"]+)\"/g,mm;while(mm=re.exec(seg)){var u=mm[1];if(out.indexOf(u)<0){out.push(u);}}if(out.length===0){var re2=/src=\"(https:\\/\\/[^\"]+\\.misskon\\.com\\/(?:uploads|images)\\/[^\"]+)\"/g,mm2;while(mm2=re2.exec(seg)){var u2=mm2[1];if(out.indexOf(u2)<0){out.push(u2);}}}if(out.length===0){return ['MISSKON_ERR:无图片 head='+body.substring(0,100)];}return out;}catch(e){return ['MISSKON_ERR:'+e.message+' | head='+String(result).substring(0,80)];}})()"};
