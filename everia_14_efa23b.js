...
class ConvertedSource_everia_14_efa23b extends ComicSource {
  name = "Everia图集";
  key = "everia_14_efa23b";
  version = "0.1.0";
  minAppVersion = "1.0.0";
  url = "";
  baseUrl = "https://everia.club";
  defaultHeaders = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "Referer": "https://everia.club"};
  legadoRules = {"ruleSearchUrl":"https://everia.club/page/searchPage/?s=searchKey","ruleSearchList":"article.entry","ruleSearchName":".entry-title a@text|.search-entry-title a@text","ruleSearchAuthor":"","ruleSearchNoteUrl":"a.thumbnail-link@href","ruleSearchCoverUrl":".thumbnail img@src","ruleSearchKind":"","ruleSearchLastChapter":"","ruleBookName":".single-post-title@text","ruleBookAuthor":"","ruleBookKind":"","ruleBookLastChapter":"","ruleIntroduce":"","ruleCoverUrl":".entry-content img@src","ruleChapterList":"@js:(function(){try{var h=String(result);var base='',max=1;var re=/href=\"(https:\\/\\/[^\"]+?)\\/\\d+\\/\"[^>]*class=\"post-page-numbers\"/g,mm;while(mm=re.exec(h)){if(!base){base=mm[1];}}var re2=/class=\"page-number\">(\\d+)</g,mm2;while(mm2=re2.exec(h)){var n=parseInt(mm2[1],10);if(n>max){max=n;}}if(!base){return ['EVERIA_ERR:no-base head='+h.substring(0,80)];}var out=[];for(var i=1;i<=max;i++){out.push({title:'第'+i+'页',url:i===1?base:base+'/'+i+'/'});}return out;}catch(e){return ['EVERIA_ERR:'+e.message+' head='+String(result).substring(0,80)];}})()","ruleChapterName":"@js:result.title","ruleChapterUrl":"","ruleContentUrl":"@js:result.url","ruleBookContent":".entry-content img@src"};
