...
class ConvertedSource_buondua_15_095853 extends ComicSource {
  ...
  legadoRules = {
  "ruleSearchUrl": "https://buondua.com/?search=searchKey&start={{(searchPage-1)*20}}",
  "ruleSearchList": "div.items-row",
  "ruleSearchName": "h2 a@text",
  "ruleSearchAuthor": "",
  "ruleSearchNoteUrl": "a.item-link@href@js:'https://buondua.com'+result",
  "ruleSearchCoverUrl": ".item-thumb img@src",
  "ruleSearchKind": "",
  "ruleSearchLastChapter": "",
  "ruleBookName": "h1@text@js:result.replace(/\\s*-\\s*\\(\\s*Page\\s+\\d+\\s*\\/\\s*\\d+\\s*\\)\\s*$/,'').trim()",
  "ruleBookAuthor": "",
  "ruleBookKind": "",
  "ruleBookLastChapter": "",
  "ruleIntroduce": "",
  "ruleCoverUrl": ".article-fulltext img@src",
  "ruleChapterList": "@js:(function(){try{var h=String(result);var base='',max=1;var c=h.match(/rel=\"canonical\" href=\"([^\"]+)\"/);if(c&&c[1]){base=c[1].replace(/\\?page=\\d+$/,'');}var re=/href=\"(\\/[^\"]+?)\\?page=(\\d+)\"/g,mm;while(mm=re.exec(h)){if(!base){base=mm[1];}var n=parseInt(mm[2],10);if(n>max){max=n;}}if(!base){return ['BUONDUA_ERR:no-base head='+h.substring(0,60)];}if(base.indexOf('http')!==0){base='https://buondua.com'+base;}var out=[];for(var i=1;i<=max;i++){out.push({title:'第'+i+'页',url:i===1?base:base+'?page='+i});}return out;}catch(e){return ['BUONDUA_ERR:'+e.message+' head='+String(result).substring(0,80)];}})()",
  "ruleChapterName": "@js:result.title",
  "ruleChapterUrl": "",
  "ruleContentUrl": "@js:result.url",
  "ruleBookContent": ".article-fulltext img@src"
};
