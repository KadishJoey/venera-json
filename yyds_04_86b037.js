...
class ConvertedSource_yyds_04_86b037 extends ComicSource {
  name = "◯ YYDS";
  key = "yyds_04_86b037";
  version = "0.1.0";
  minAppVersion = "1.0.0";
  url = "";
  baseUrl = "http://www.yydsmh.com";
  defaultHeaders = {"User-Agent": "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36", "Referer": "http://www.yydsmh.com"};
  legadoRules = {"ruleSearchUrl":"http://www.yydsmh.com/search?searchkey=searchKey@Header:{Cookie=articlevisited=1}","ruleSearchList":"id.js_comicSortList@li|\n.card|\n.comic-item","ruleSearchName":"class.title@text|\n.card-title@text\n@js:\njava.put(\"name\",result);","ruleSearchAuthor":"","ruleSearchNoteUrl":"a@href","ruleSearchCoverUrl":"img@src#https#http\n@js:\njava.put(\"img\",result);","ruleSearchKind":"","ruleSearchLastChapter":"class.chapter@text|\n.tags-list@text|\n.card-text@text","ruleBookName":"","ruleBookAuthor":"class.author@text","ruleBookKind":"class.type@text","ruleBookLastChapter":"","ruleIntroduce":"id.js_desc_content@text","ruleCoverUrl":"","ruleChapterList":"id.js_chapters@li","ruleChapterName":"a@text\n@js:\na = \"  \" + result","ruleChapterUrl":"","ruleContentUrl":"a@href","ruleBookContent":"@js:\n//java.log(src)\np=\"https://www.yydsmh.com/api/comic/read/pics\";\na=String(baseUrl).match(/(\\d+)/g);\nfig=/<figure[\\s\\S]*?<\\/figure>/g;\nye=String(result).match(fig).length;\nye=ye%10!=0?(ye/10+1):(ye/10);\nid=a[1];    aid=a[0];    pic=[];\njava.log(ye)\nfunction PIC(id,aid)\n{\n    offset=pic.length;\n    java.log(offset)\nbody=\"id=\"+id+\"&aid=\"+aid+\"&offset=\"+offset+\"&limit=10\";\n    pics=java.post(p,body,{}).body();\n    img=JSON.parse(pics).data.pic;\n    \n    for(P=0;P<img.length;P++)\n    {\n        pic.push(img[P].pic);\n    }\n}\nfor(i=0;i<ye;i++)\n{\n    PIC(id,aid);\n}\npic@Header:{Referer:host}"};
