...
class ConvertedSource_webtoon_us_68_3131e7 extends ComicSource {
  name = "Webtoon_us";
  key = "webtoon_us_68_3131e7";
  version = "0.1.0";
  minAppVersion = "1.0.0";
  url = "";
  baseUrl = "https://us.webtoons.com";
  defaultHeaders = {"User-Agent": "Mozilla/5.0 (Windows NT 6.1; WOW64; rv:34.0) Gecko/20100101 Firefox/34.0", "Referer": "https://us.webtoons.com"};
  legadoRules = {"ruleSearchUrl":"https://us.webtoons.com/zh-hant/search?keyword=searchKey@Header:{Cookie:\"locale=zh_TW;pagGDPR=true\",Referer:\"https://us.webtoons.com/zh-hant/search\"}","ruleSearchList":"@js:\nvar doc = org.jsoup.Jsoup.parse(result);\nif (baseUrl.match(/search/))\n  doc.select('ul.card_lst li')\nif (baseUrl.match(/UPDATE/))\n  doc.select('.challenge_lst>ul li')\nif (baseUrl.match(/MONDAY/))\n  doc.select('#dailyList>div:nth-of-type(1) li')\nif (baseUrl.match(/TUESDAY/))\n  doc.select('#dailyList>div:nth-of-type(2) li')\nif (baseUrl.match(/WEDNESDAY/))\n  doc.select('#dailyList>div:nth-of-type(3) li')\nif (baseUrl.match(/THURSDAY/))\n  doc.select('#dailyList>div:nth-of-type(4) li')\nif (baseUrl.match(/FRIDAY/))\n  doc.select('#dailyList>div:nth-of-type(5) li')\nif (baseUrl.match(/SATURDAY/))\n  doc.select('#dailyList>div:nth-of-type(6) li')\nif (baseUrl.match(/SUNDAY/))\n  doc.select('#dailyList>div:nth-of-type(7) li')\nif (baseUrl.match(/COMPLETE/))\n  doc.select('#completed+ul li')\nif (baseUrl.match(/DRAMA/))\n  doc.select('.card_wrap>ul:nth-of-type(1) li')\nif (baseUrl.match(/FANTASY/))\n  doc.select('.card_wrap>ul:nth-of-type(2) li')\nif (baseUrl.match(/COMEDY/))\n  doc.select('.card_wrap>ul:nth-of-type(3) li')\nif (baseUrl.match(/ACTION/))\n  doc.select('.card_wrap>ul:nth-of-type(4) li')\nif (baseUrl.match(/SLICE_OF_LIFE/))\n  doc.select('.card_wrap>ul:nth-of-type(5) li')\nif (baseUrl.match(/ROMANCE/))\n  doc.select('.card_wrap>ul:nth-of-type(6) li')\nif (baseUrl.match(/SUPER_HERO/))\n  doc.select('.card_wrap>ul:nth-of-type(7) li')\nif (baseUrl.match(/HEARTWARMING/))\n  doc.select('.card_wrap>ul:nth-of-type(8) li')\nif (baseUrl.match(/HISTORICAL/))\n  doc.select('.card_wrap>ul:nth-of-type(9) li')\nif (baseUrl.match(/THRILLER/))\n  doc.select('.card_wrap>ul:nth-of-type(10) li')\nif (baseUrl.match(/SPORTS/))\n  doc.select('.card_wrap>ul:nth-of-type(11) li')\nif (baseUrl.match(/HORROR/))\n  doc.select('.card_wrap>ul:nth-of-type(12) li')","ruleSearchName":"class.subj@text","ruleSearchAuthor":"class.author@text","ruleSearchNoteUrl":"tag.a@href","ruleSearchCoverUrl":"tag.img@src@Header:{Referer:thisPage}","ruleSearchKind":"class.genre@text","ruleSearchLastChapter":"","ruleBookName":"class.subj.0@text","ruleBookAuthor":"class.author@text","ruleBookKind":"class.info@class.genre@text","ruleBookLastChapter":"id._listUl@class.subj.0@text","ruleIntroduce":"class.summary@text","ruleCoverUrl":"class.thmb.0@tag.img@src@Header:{Referer:thisPage}","ruleChapterList":":_H006_qQix0Uw66aG6ckKzOkzOS3p7TZJlxD46oyRuY73SchE=\n","ruleChapterName":"img@alt","ruleChapterUrl":"","ruleContentUrl":"tag.a@href","ruleBookContent":":_H006_3mcXpOqYmCJJH1Okhg8MHZauuSqBmF41ibyy/ZIs7Gnmz6VOaI7IidBD0IW2vEVWQNvKBJJVnDdD\nV6WLL0QqSA==\n"};
