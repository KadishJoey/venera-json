...
class ConvertedSource_source_06_b7b48e extends ComicSource {
  ...
  headers = {
    ...
    "Referer": "https://v4api.zaimanhua.com"
  };

  legadoRules = {
  "ruleSearchUrl": "https://v4api.zaimanhua.com/app/v1/search/index?keyword=searchKey&source=0&page=searchPage&size=20",
  "ruleSearchList": "$.data.list||$.data",
  "ruleSearchName": "$.title",
  "ruleSearchAuthor": "$.authors",
  "ruleSearchNoteUrl": "$.comic_id&&$.id<js>(function(result)\n{\nvar text = new String(result); \nif (text.startsWith(\"0\"))\n{ \n  text=text.substring(1);\n} \nelse \n{\n  text=text.substring(text.length-1);\n} \nvar timestamp = new Date().getTime();\nreturn \"https://v4api.zaimanhua.com/app/v1/comic/detail/\" + text + \"?platform=android&timestamp=\" + timestamp + \"&_v=2.3.0&_c=101_01_01_000\";\n})(result);</js>@Header:{Authorization:\"Bearer 在此处输入你的token，token需抓包获得\",appversion:\"2.3.0\",channel=\"101_01_01_000\",buildnumber=\"1502206\",host=\"v4api.zaimanhua.com\"}",
  "ruleSearchCoverUrl": "$.cover",
  "ruleSearchKind": "$.types",
  "ruleSearchLastChapter": "$.last_name",
  "ruleBookName": "$.data.data.title",
  "ruleBookAuthor": "",
  "ruleBookKind": "$.data.data.status&&$.data.data.types<js>(function(result){\n     var sp = new String(result).split(\",\");\n     var l = [];\n     for (var i = 0; i < sp.length; i++){\n          var current = sp[i];\n          current = current.trim();\n          var index = current.indexOf(\"=\");\n          var key = current.slice(0, index).replace(\"{\",\"\").replace(\" \",\"\");\n          if (key != \"tag_name\"){\n              continue;\n          }\n          var value = current.slice(index + 1).replace(\"}\",\"\");\n          l.push(value);\n     }\n     return l;\n})(result);\n</js>",
  "ruleBookLastChapter": "$.data.data.last_update_chapter_name",
  "ruleIntroduce": "$.data.data.description",
  "ruleCoverUrl": "$.data.data.cover",
  "ruleChapterList": "-$.data.data.chapters.*.data.*",
  "ruleChapterName": "$.chapter_title",
  "ruleChapterUrl": "",
  "ruleContentUrl": "$.chapter_id<js>(function(result,prePage){\n     var prePageUrl = new String(prePage);\n     var first = prePageUrl.split(\"?\")[0].replace(\"detail\",\"chapter\");\n     var timestamp = Math.floor(new Date().getTime() / 1000);\n     return first + \"/\" + result + \"?platform=android&timestamp=\" + timestamp + \"&_v=2.3.0&_c=101_01_01_000\";\n})(result,baseUrl);</js>",
  "ruleBookContent": "*.data.page_url.*"
};
