...
class ConvertedSource_18_12_5158bf extends ComicSource {
  name = "18真人图集";
  key = "18_12_5158bf";
  version = "0.1.0";
  minAppVersion = "1.0.0";
  url = "";
  baseUrl = "https://18gallery.com";
  defaultHeaders = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36", "Referer": "https://18gallery.com"};
  legadoRules = {"ruleSearchUrl":"https://18gallery.com/popular/?page=searchPage","ruleSearchList":"a[href*='/gallery/']","ruleSearchName":"img@alt","ruleSearchAuthor":"","ruleSearchNoteUrl":"href","ruleSearchCoverUrl":"img@src","ruleSearchKind":"","ruleSearchLastChapter":"","ruleBookName":"h1[class*=article-title]@text","ruleBookAuthor":"","ruleBookKind":"","ruleBookLastChapter":"","ruleIntroduce":"","ruleCoverUrl":"img[src*='img.18gallery.com']@src","ruleChapterList":"@js:(function(){var base=result.match(/rel=\"canonical\" href=\"([^\"]+)\"/)[1];var max=1,re=/\\/gallery\\/[^\\/]+?\\/page\\/(\\d+)\\//g,mm;while(mm=re.exec(result)){var n=parseInt(mm[1]);if(n>max){max=n;}}var out=[];for(var i=1;i<=max;i++){out.push({title:'第'+i+'页',url:base+'page/'+i+'/'});}return out;})()","ruleChapterName":"@js:result.title","ruleChapterUrl":"","ruleContentUrl":"@js:result.url","ruleBookContent":"@js:(function(){try{var out=[];var re=/src=\"(https:\\/\\/img\\.18gallery\\.com\\/[^\"]+)\"/g,mm;while(mm=re.exec(result)){var u=mm[1];if(out.indexOf(u)<0){out.push(u);}}if(out.length===0){return ['18GAL_ERR:无图片 head='+String(result).substring(0,100)];}return out;}catch(e){return ['18GAL_ERR:'+e.message+' | head='+String(result).substring(0,80)];}})()"};
